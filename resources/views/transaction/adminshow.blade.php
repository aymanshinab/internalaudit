...
<x-app-layout>
    ...
    @endif
    @if (session('fill'))
    <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative flex flex-row-reverse justify-between"
        role="alert">
        <div>
            <span class="absolute top-0 bottom-0 left-0 px-4 py-3">
                <svg id="close-icon" class="fill-current h-6 w-6 text-red-500" role="button"
                    xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                    <title>Close</title>
                    <path
                        d="M14.348 14.849a1.2 1.2 0 0 1-1.697 0L10 11.819l-2.651 3.029a1.2 1.2 0 1 1-1.697-1.697l2.758-3.15-2.759-3.152a1.2 1.2 0 1 1 1.697-1.697L10 8.183l2.651-3.031a1.2 1.2 0 1 1 1.697 1.697l-2.758 3.152 2.758 3.15a1.2 1.2 0 0 1 0 1.698z" />
        ...
        <div>
            <strong class="block sm:inline">{{ session('fill') }}</strong>
        </div>
    </div>
    <script>
        document.getElementById('close-icon').addEventListener('click', function() {
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div>
                    <div class= " pb-10 p-4 text-xl font-normal flex justify-between  ">
                        ...
                        <form action="{{ route('transaction.adminupdate', $transaction->id) }}" method="POST">
                            @method('PATCH')
                            @csrf
                    ...
                    <div>
                        ...
                        <div class="pr-3">
                            <x-primary-button> <a href="{{ route('transaction.adminindex', $transaction->id) }}">رجوع</a>
                            </x-primary-button>
                        </div>
                                ...
                                <td> {{ $procedure->to_name }}</td>
                                    <td>
                                        @if ($procedure->procedure_name == 'رفض')
                                            <a href="{{ Route('notice.adminshow', $transaction->id) }}">
